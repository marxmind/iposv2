...
public class XtrasBean implements Serializable{
	...
	private List<Xtras> xtras = new ArrayList<Xtras>();
	...
	private List<Xtras> xtraSelected = new ArrayList<Xtras>(); 
	...
	public void init(){
		...
		List<Xtras> xtrasTmp = new ArrayList<Xtras>();
	...
	public void xtraSave(Xtras xtra){
		...
		if(xtra.getTransType()==MoneyStatus.ALLOWANCES.getId()){
			...
			io.setOutAmount(xtra.getAmount());
			io.setReceiptNo("0000-00-00-0000000000");
		}else if(xtra.getTransType()==MoneyStatus.CASH_OUT.getId()){
			io.setTransType(MoneyStatus.CASH_OUT.getId());
			io.setDescripion(xtra.getDescription());
			io.setOutAmount(xtra.getAmount());
			io.setReceiptNo("0000-00-00-0000000000");
		}else if(xtra.getTransType()==MoneyStatus.CASH_IN.getId()){
			io.setTransType(MoneyStatus.CASH_IN.getId());
			io.setDescripion(xtra.getDescription());
			io.setInAmount(xtra.getAmount());
			io.setReceiptNo("0000-00-00-0000000000");
		}	
	...
	public List getTransTypes() {
		...
		transTypes.add(new SelectItem(MoneyStatus.CASH_OUT.getId(), MoneyStatus.CASH_OUT.getName()));
		transTypes.add(new SelectItem(MoneyStatus.CASH_IN.getId(), MoneyStatus.CASH_IN.getName()));
		return transTypes;
	}
