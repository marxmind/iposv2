...
public class AccountingBean implements Serializable {
	...
	private List<Incomes> soldItems = new ArrayList<Incomes>();
	
	private List<DeliveryItemReceipt> receipts = new ArrayList<DeliveryItemReceipt>();
	...
	private List<CustomerPaymentTrans> cashs = new ArrayList<CustomerPaymentTrans>();
	...
	private List<MoneyIO> ios = new ArrayList<MoneyIO>();
	...
	private List<Summary> summaryData = new ArrayList<>();
	...
	private List<ProductInventory> warehouseProduct = new ArrayList<ProductInventory>();
	private double warehouseTotalQty;
	private List<DeliveryItem> returnItems = new ArrayList<DeliveryItem>();
	...
	private List<StoreProduct> stores = new ArrayList<StoreProduct>();
	...
	public void startCash(){
		...
		cashs = new ArrayList<CustomerPaymentTrans>();
	...
	public void startReceipts(){
		...
		receipts = new ArrayList<DeliveryItemReceipt>();
	...
	public void startIncomes(){
		soldItems = new ArrayList<Incomes>();
		loadSoldProductOnly();
	...
	public void loadSoldProductOnly(){
		...
		Map<String, DeliveryItemTrans> product = new HashMap<String, DeliveryItemTrans>();
		Map<Long, Map<String, DeliveryItemTrans>> products = new HashMap<Long,Map<String, DeliveryItemTrans>>();
		...
		for(DeliveryItemTrans trn : DeliveryItemTrans.retrieve(tran)){
			...
			if(products!=null && products.size()>0){
				...
				if(products.containsKey(id)){
					...
					product = new HashMap<String, DeliveryItemTrans>();
	...
	public void loadSoldGrocerry() {
		...
		double price=0d,capital = 0d, sales=0d, net =0d;
		int idKey=getSoldItems().size() + 1;
		List<Incomes> listIncome = new ArrayList<Incomes>();
		Map<String, List<Incomes>> incomeMap = new HashMap<String, List<Incomes>>();
		for(String date : mapSold.keySet()) {
			listIncome = new ArrayList<Incomes>();
	...
	public void loadSoldGrocerryOnly(){
		...
		Map<String, PurchasedItem> product = new HashMap<String, PurchasedItem>();
		Map<Long, Map<String, PurchasedItem>> products = new HashMap<Long,Map<String, PurchasedItem>>();
		...
		for(PurchasedItem items : PurchasedItem.retrieve(item)){
			...
			if(products!=null && products.size()>0){
				...
				if(products.containsKey(id)){
					...
					product = new HashMap<String, PurchasedItem>();
				...
				products = new HashMap<Long,Map<String, PurchasedItem>>();
				product = new HashMap<String, PurchasedItem>();
	...
	public void retrieveStoreRemainingProduct(){
		...
		stores = new ArrayList<StoreProduct>();
	...
	public void retrieveRemainingProductInDeliveryTruck(){
		...
		if(items!=null && itemDate==null & isNotExist){
			...
			Map<Long, DeliveryItem> product = new HashMap<Long, DeliveryItem>();
	...
	private void productRemainingOnDeliveryTruck(){
		returnItems = new ArrayList<DeliveryItem>();
		DeliveryItem item = new DeliveryItem();
		item.setIsActive(1);
		item.setStatus(DeliveryStatus.POSTED_FOR_DELIVERY.getId());
		item.setBetween(true);
		...
		Map<Long, Map<String, DeliveryItem>> products = new HashMap<Long,Map<String, DeliveryItem>>();
		Map<String, DeliveryItem> product = new HashMap<String, DeliveryItem>();
		...
		for(DeliveryItem it : DeliveryItem.retrieve(item)){
			...
			if(products!=null && products.size()>0){
				...
				if(products.containsKey(id)){
					...
					product = new HashMap<String, DeliveryItem>();
					ProductProperties prop = ProductProperties.properties(it.getProduct().getProductProperties().getPropid()+"");
					it.getProduct().setProductProperties(prop);
		...
		Map<Long, DeliveryItem> unSort = new HashMap<Long, DeliveryItem>();
	...
	private void warehouseReaminingProduct(){
		warehouseProduct = new ArrayList<ProductInventory>();
	...
	private void storeProduct(){
		...
		List<Summary> summaryDetailed = new ArrayList<Summary>();
		Map<String, List<Summary>> sumDataDetailed = new HashMap<String, List<Summary>>();
		...
		Map<Long, PurchasedItem> unSort = new HashMap<Long, PurchasedItem>();
		for(Long id : products.keySet()){
		...
		//detailed
			...
			List<PurchasedItem> prodList = new ArrayList<PurchasedItem>();
			Map<String, List<PurchasedItem>> prods = new HashMap<String, List<PurchasedItem>>();
			...
			for(PurchasedItem items : PurchasedItem.retrieve(item)){
				...
				if(prods!=null && prods.size()>0) {
					...
					if(prods.containsKey(date)) {
						...
						prodList = new ArrayList<PurchasedItem>();
						prodList.add(items);
						prods.put(date, prodList);
					}
				...
				for(PurchasedItem items : sort.get(date)){
					...
					if(sumDataDetailed!=null && sumDataDetailed.size()>0) {
						if(sumDataDetailed.containsKey(items.getDatesold())) {
							...
							summaryDetailed = new ArrayList<Summary>();
							summaryDetailed.add(sum);
							sumDataDetailed.put(items.getDatesold(), summaryDetailed);
						}
					}else {
		...
		if(getSummaryFilterId()==2){ //detailed
			setSummaryData(new ArrayList<Summary>());
	...
	private void delivery(){
		...
		Map<Long, DeliveryItemTrans> unSort = new HashMap<Long, DeliveryItemTrans>();
		for(Long id : products.keySet() ){
	...
	private List<Reports> filterPerMunicipalityFromReceipts(String dateFrom, String dateTo, List<Reports> reports){
		...
		Map<Integer, DeliveryItemReceipt> municipalityMap = new HashMap<Integer, DeliveryItemReceipt>();
		for(DeliveryItemReceipt recpts : DeliveryItemReceipt.retrieve(rcpt, cus)){
			...
			if(municipalityMap!=null && municipalityMap.size()>0){
				...
				municipalityMap = new HashMap<Integer, DeliveryItemReceipt>();
				municipalityMap.put(id, recpts);
			}
		...
		//filter per barangay
		Map<Integer, DeliveryItemReceipt> barangay = new HashMap<Integer, DeliveryItemReceipt>();
		Map<Integer, Map<Integer, DeliveryItemReceipt>> barangayData = new HashMap<Integer, Map<Integer, DeliveryItemReceipt>>();
		...
		for(DeliveryItemReceipt rpt : DeliveryItemReceipt.retrieve(rcpt, cus)){
			...
			if(barangayData!=null && barangayData.size()>0){
				...
				if(barangayData.containsKey(munId)){
					...
					barangay = new HashMap<Integer, DeliveryItemReceipt>();
				...
				barangayData = new HashMap<Integer, Map<Integer, DeliveryItemReceipt>>();
				barangay = new HashMap<Integer, DeliveryItemReceipt>();
				barangay.put(barId, rpt);
				barangayData.put(munId, barangay);
			}
